...
function login(name: string, lastName: string, contato: number) {
  obj3.name = name;
...
function verificandoCadastro(name: string) {
  if (obj3.name === name) {
...
const turmaA: string[] = ["Joao", "Maria", "Flavio", "Calos"];
const turmaB: string[] = ["Carlos", ...turmaA, "Pedro", "Mateus"];
...
const numeros: number[] = retornaArray(1, 2, 4, 12, 20, 80, 60, 50);
...
const /**Exemplo de tupla ====>**/ tupla: [number, string, boolean] = [
    1,
    "abc",
    false,
  ];
...
console.log(
  `the vehicle has an engine ${motor}, and was launched in the year of ${anoDeFabricacao}`
);
...
const dizerOla = (nome: string = "Pessoa") =>
  !nome ? (nome = "Pessoa") : console.log(`Ola, ${nome}`);
